        ...
        <script>
            const app = Vue.createApp({
                ...
                methods: {
                    ...
                    async loadList() {
                        const data = await this.request("/AutoBudget-2/api/users", "GET");
                        if(data) {
                            this.list = data.list;
                        }
                    },
                    async addUser() {
                        const data = await this.request("/AutoBudget-2/api/users", "POST", {login: this.newLogin, name: this.newName, role: this.newRole, password: this.newPassword});
                        if(data) {
                            this.newRole = 'USER'; 
                            this.newLogin = ''; 
                            this.newMName = ''; 
                    ...
                    async removeUser(id) {
                        const data = await this.request("/AutoBudget-2/api/users?id="+id, "DELETE");
                        if(data) {
                            await this.loadList();
                        }
                    }
